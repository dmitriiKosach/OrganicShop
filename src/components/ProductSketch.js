...
const ProductSketch = (props) => {
  ...
  const updateReserveProduct = (p) => {
    product.unit === "gram"
      ? (product.reserve -= p.quantity / 1000).toFixed(2)
      : (product.reserve -= p.quantity);
  ...
  const addProductToList = (product) => {
    const ind = props.productList.findIndex(
      ...
      <div className="sketch__item">
        <div className="sketch__wrapper">
          {userData.isAdmin ? (
            props.removeProduct ? (
              <button
                onClick={() => removeProductFn(product)}
                className="button__remove-product"
                type="button"
              >
          ...
          <div className="sketch__content">
            ...
            <div className="sketch__content-property">
              <div className="property__reserve">
                <span>
                  Available: {Number.parseInt(product.reserve).toFixed(2)}{" "}
                  {product.unit === "gram" ? "kg" : product.unit}
                </span>
              </div>
            ...
            <div className="sketch__content-buttons">
              <div className="button__quantity">
                ...
                <button
                  ...
                  disabled={
                    product.unit === "gram"
                      ? count / 1000 >= product.reserve
                      : count >= product.reserve
                  }
                  onClick={addStep}
                  type="button"
                >
